					...
					<div class="col-lg-3 col-sm-6">
						<div class="card card-plain w-100">
							...
							<div class="card-body px-0">
								<h5>
									<a href="{{ route('front.article.detail',['slug' => $article?->slug]) }}" class="text-dark font-weight-bold">{{ $article->title }}</a>
								</h5>
								<p class="text-justify">{{ Str::limit(strip_tags($article?->description), 150, '...') }}</p>
								<a href="{{ route('front.article.detail',['slug' => $article?->slug]) }}" class="text-info text-sm icon-move-right">Read More

...
class HalamansDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query, Request $request): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', 'vendor.adminlte.halamans.datatables_action')
            ...
            ->addColumn('slug', function($content){
				return '<a href="'.route('front.page.detail',['slug' => $content->slug]).'" target="_blank" title="Buka Ditab baru">'.$content->slug.' <i class="fa fa-arrow-circle-right" aria-hidden="true"></i></a>';
			})
            /* ->addColumn('getKategori', function (PageModel $perumahan) {
                return $perumahan->getKategori->title;
            }) */
            /* ->editColumn('kategori', function (PageModel $perumahan) {
     ...
     */
    public function query(PageModel $model,Request $request): QueryBuilder
    {
        $q =  $model->newQuery();
    ...
    public function getColumns(): array
    {
        return [
			...
			Column::make('template')->title('Template')->searchable(true)->visible(true),

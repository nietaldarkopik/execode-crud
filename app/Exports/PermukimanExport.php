...
class PermukimanExport implements FromCollection, WithHeadings
{
    protected $filters;
    ...
    {
        $q = PermukimanModel::query();

		$q->select([
			DB::raw('(SELECT name FROM regencies WHERE permukiman.kabkota_id = regencies.id) as kabkota_id'),
			DB::raw('(SELECT name FROM districts WHERE permukiman.kecamatan_id = districts.id) as kecamatan_id'),
			DB::raw('(SELECT name FROM villages WHERE permukiman.kelurahan_id = villages.id) as kelurahan_id'),
			'Nama_Permukiman',
			//'Nama_Pengembang',
			//'Telepon_Pengembang',
			//'Email_Pengembang',
        ...
        if (!empty($this->filters['search']['value'])) {
            ...
            $q->where(function($q) use ($searchValue) {
                $q->where('nama_permukiman', 'like', "%{$searchValue}%")

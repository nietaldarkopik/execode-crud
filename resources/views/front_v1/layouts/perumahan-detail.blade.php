        ...
        <div class="row entry entry-single portfolio-details bg-white">
                    ...
                    <div class="swiper-wrapper align-items-center">
						...
						@if(!empty($perumahan->siteplan))
							...
							@if(in_array($mimeType, $allowedMimeTypes))
								<div class="swiper-slide">
									<img src="{{ asset(Storage::url($perumahan->siteplan))}}" class="w-100 object-fit-cover" style="height: 500px;" alt="">
								</div>
							@endif
						@endif
						...
						@if(!empty($perumahan->photo))
							...
							@if(in_array($mimeType, $allowedMimeTypes))
								<div class="swiper-slide">
									<img src="{{ asset(Storage::url($perumahan->photo))}}" class="w-100 object-fit-cover" style="height: 500px;" alt="">
								</div>
							@endif
						@endif
                                    ...
                                    <div id="listing-overview" class="listing-section">
										...
										<div class="row my-3">
											<div class="col-xs-12 col-sm-12">
												...
												<p>{{ (!empty($perumahan->nama_perumahan)) ? $perumahan->nama_perumahan : '-' }}</p>
											</div>
											...
											<div class="col-xs-12 col-sm-6">
												...
												<p>
													...
													$query->where('province_id',63); })->where('id','=',$perumahan->kecamatan_id ??
													'')->get()->first()?->name }}
												</p>				
											</div>
											<div class="col-xs-12 col-sm-6">
											...
											<div class="col-xs-12 col-sm-6">
												...
												<p>{{ (!empty($perumahan->alamat)) ? $perumahan->alamat : '-' }}</p>
											</div>
											<div class="col-xs-12 col-sm-6">
												<h6 class="mb-0"><strong>Luas</strong></h6>
												<p>{{ (!empty($perumahan->luas)) ? $perumahan->luas : '-' }}</p>
											</div>
											<div class="col-xs-12 col-sm-6">
												<h6 class="mb-0"><strong>Tahun Siteplan</strong></h6>
												<p>{{ (!empty($perumahan->tahun_siteplan)) ? $perumahan->tahun_siteplan : '-' }}</p>
											</div>
											<div class="col-xs-12 col-sm-6">
												<h6 class="mb-0"><strong>No Bast</strong></h6>
												<p>{{ (!empty($perumahan->no_bast)) ? $perumahan->no_bast : '-' }}</p>
											</div>
											<div class="col-xs-12 col-sm-6">
												<h6 class="mb-0"><strong>Total Unit</strong></h6>
												<p>{{ (!empty($perumahan->total_unit)) ? $perumahan->total_unit : '-' }}</p>
											</div>
											<div class="col-xs-12 col-sm-6">
												<h6 class="mb-0"><strong>Latitude</strong></h6>
												<p>{{ (!empty($perumahan->latitude)) ? $perumahan->latitude : '-' }}</p>
											</div>
											<div class="col-xs-12 col-sm-6">
												<h6 class="mb-0"><strong>Longitude</strong></h6>
												<p>{{ (!empty($perumahan->longitude)) ? $perumahan->longitude : '-' }}</p>
											</div>
										</div>
                            ...
                            <div id="listing-facilities" class="listing-section">
                                ...
                                <div class="row">
                                    <div class="col-md-12">
                                        ...
                                        Nomor : {{ (isset($perumahan->file_bast) && !empty($perumahan->file_bast))?$perumahan->file_bast :'-' ?? '-' }}
                                        <br>
										...
										if (!empty($perumahan->file_bast) && file_exists(storage_path('app/public/'.$perumahan->file_bast))) {
											...
											if($imageInfo !== false)
											{
												...
												echo '<a href="'.asset(Storage::url($perumahan->file_bast)).'"
                                ...
                                <div class="row mt-3">
                                    <div class="col-md-12">
                                        ...
                                        <table class="table table-hover table-bordered table-utility-facility">
                                            ...
                                            <tbody>
												@php
												$psu = App\Models\PsuPerumahanModel::where('id_perumahan', $perumahan->id)->get();
												@endphp
												@if($psu->count() > 0)
													@foreach($psu as $i => $p)
							...
							<div class="row mt-3">
								<div class="col-md-12">
									...
									<table class="table table-hover table-bordered table-utility-facility">
										...
										<tbody>
											@php
											$dokumen = App\Models\PerumahanDokumenModel::where('id_perumahan', $perumahan->id)->get();
											@endphp
											@if($dokumen->count() > 0)
            ...
            <div class="col-lg-4">
                <div class="portfolio-info">
                    ...
                    <ul>
                        <li><strong><i class="bi bi-building"></i></strong> {{ (!empty($perumahan->nama_pengembang))?$perumahan->nama_pengembang : '-' ?? '-'}}</li>
                        <li><strong><i class="bi bi-telephone-fill"></i></strong> {{ (!empty($perumahan->telepon_pengembang))?$perumahan->telepon_pengembang : '-' ?? '-'}}</li>
                        <li><strong><i class="bi bi-envelope-fill"></i></strong> {{ (!empty($perumahan->email_pengembang))?$perumahan->email_pengembang : '-' ?? '-'}}</li>
...
<script>
	$(document).ready(function(){
		var map = L.map('map').setView([{{ $perumahan->latitude }}, {{ $perumahan->longitude }}], 12); // Atur sesuai koordinat daerah Anda

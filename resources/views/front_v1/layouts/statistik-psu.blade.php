...
$totalPsuPerumahan = \App\Models\KabupatenKotaModel::where('province_id','=',63)->withCount(['getPerumahan as total_psu_perumahan' => function($query){
	$query->join('psu_perumahan','perumahan.id','=','psu_perumahan.id_perumahan')
	->selectRaw('count(distinct psu_perumahan.id)')
	->groupBy('kabkota_id');
}])->get();
$totalPerumahanBast = \App\Models\KabupatenKotaModel::where('province_id','=',63)->withCount(['getPerumahan as total_sudah_bast' => function($query){
	$query->where('no_bast','!=','');
	$query->where('no_bast','!=','-');
...
<script>
	...
	var labelJumlahPerumahanBast = totalPerumahanBast.map((v, i, a) => {return (!v.name)? 0 : v.name;});
	var valueJumlahPerumahanSudahBast = totalPerumahanBast.map((v, i, a) => {return (!v.total_sudah_bast)? 0 : v.total_sudah_bast;});
	var valueJumlahPerumahanBelumBast = totalPerumahanBast.map((v, i, a) => {return (!v.total_belum_bast)? 0 : v.total_belum_bast;});
...
<script>
	...
	var pieJumlahPerumahanTabletmp = totalPerumahan.map((v, i, a) => {
	  ...
	  return '<tr><td>' + ((i + 1).toString()) + '</td><td class="text-start"><span style="background-color: ' + colors[i] + ' !important; width: 10px; height: 10px; padding:2px; display:inline-block;"></span> ' + v.name + '</td><td>' + (v.get_perumahan_count ?? 0) + '</td></tr>';
  });
  $("#pieJumlahPerumahanTable").html('<table class="table table-striped table-bordered"><thead><tr><th>No</th><th>Kab/Kota</th><th>Jumlah</th></tr></thead><tbody>' + pieJumlahPerumahanTabletmp.join('') + '</tbody><tfoot><tr><td class="text-center" colspan="2"><strong>Total Perumahan</strong></td><td><strong>' + (sumtotalPerumahan) + '</strong></td></tr></tfoot></table>');
...
<script>
  ...
  var psuCarttmp = totalPerumahanBast.map((v, i, a) => {
	sumTotalSudahBast += (!v.total_sudah_bast)?0:v.total_sudah_bast;
	sumTotalBelumBast += (!v.total_belum_bast)?0:v.total_belum_bast;
    return '<tr><td>' + ((i + 1).toString()) + '</td><td class="text-start">' + v.name + '</td><td>' + v.total_sudah_bast + '</td><td>' + v.total_belum_bast + '</td><td>' + ((v.total_sudah_bast + v.total_belum_bast).toString()) + '</td></tr>';
  });
  $("#psuTable").html('<table class="table table-striped table-bordered"><thead><tr><th>No</th><th>Kab/Kota</th><th>Sudah BAST</th><th>Belum BAST</th><th>Total</th></tr></thead><tbody>' + psuCarttmp.join('') + '</tbody><tfoot><tr><td class="text-center" colspan="2"><strong>Total</strong></td><td><strong>' + sumTotalSudahBast + '</strong></td><td><strong>' + sumTotalBelumBast + '</strong></td><td><strong>' + ((sumTotalSudahBast + sumTotalBelumBast).toString()) + '</strong></td></tr></tfoot></table>')

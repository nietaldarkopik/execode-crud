			...
			<div class="row ">
				<div class="col-md-12">
					<div class="home-search-carousel carousel-not-readyx">
						@foreach (\App\Models\SliderModel::where('status', '=', 'active')->get() as $i => $slider)
						<div class="card h-100 w-100 bg-dark">
							<img class="card-img w-100 h-100 object-fit-cover opacity-5" src="{{ asset(Storage::url($slider->image)) }}" alt="{!! $slider->judul !!}" />
							<div class="card-img-overlay d-flex justify-content-center flex-column text-center px-5 text-dark">
								<h4 class="logo-brand fs-4 text-nowrap">&copy;<span>{{ Modules\Setting\App\Models\SettingModel::where('code','global.web_title')->first()->value }}</span></h4>
								
								<h3 class=" text-light text-strong">{!! $slider->judul !!}</h3>
								<h3>{!! $slider->keterangan !!}</h3>
							</div>
						</div>
						@endforeach
					</div>

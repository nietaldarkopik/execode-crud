...
@foreach($data as $i => $d)
	<div class="card card-accordion-items" data-id="{{ $d->id }}" data-id="{{ $d->id }}" data-parent_id="{{ $d->parent_id }}">
		<div class="card-header pr-0" role="tab" id="sectionHeader{{$d->id}}">
			<h5 class="mb-0 d-flex justify-content-between pr-2">
				...
				<div class="card-tools float-end">
					<form method="post" action="{{ route('admin.menu.destroy',['menu' => $d])}}">
						@csrf
						@method('delete')
						<a data-tooltip="tooltip" title="Edit" href="{{ route('admin.menu.edit',['menu' => $d])}}" class="btn btn-sm btn-primary"
		...
		<div id="sectionContent{{ $d->id }}" class="collapse in" role="tabpanel" aria-labelledby="sectionHeader{{$d->id}}">
			<div class="card-body pr-0">
				@include('vendor.adminlte.menus.accordion',$subdata)
			</div>
		</div>
	</div>
@endforeach

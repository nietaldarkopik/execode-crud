<div class="container-fluid">
    <div class="row">
        <div class="col-md-12">
            <form action="{{ route('admin.perumahan.uploadDokumen', ['perumahan' => $perumahan]) }}"
                                            ...
                                            @foreach (\App\Models\PerumahanDokumenModel::where('id_perumahan','=',$perumahan->id)->get() as $i => $f)
                                                <li class="list-group-item row d-flex">
                                                    ...
                                                    <div class="col-sm-2">
                                                        ...
                                                        <button type="button" class="btn btn-danger btn-sm btn-remove-file-document" data-id="{{ $perumahan->id }}" data-id_dokumen="{{ $f->id }}" data-file="{{ basename($f->nama_file) }}">
...
<script>
    ...
    $(".dropzone-document").dropzone({
        url: '{{ route('admin.perumahan.uploadDokumen', ['perumahan' => $perumahan]) }}',
        maxFilesize: 512, // MB
        dataType: "json",
        addRemoveLinks: true,
        headers: {
        ...
        success: function(file, response) {
            ...
            addFileToListDokumen(file,$(this)[0].clickableElements,response,{{ $perumahan->id ?? 0}});
        },

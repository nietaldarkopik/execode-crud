<div class="container-fluid">
    <div class="row">
        <div class="col-md-12">
            <form action="{{ route('admin.perumahan.uploadPeta', ['perumahan' => $perumahan]) }}"
                                ...
                                <div class="card card-primary border-primary">
                                    ...
                                    <div class="card-body">
                                        ...
                                        <ul class="list-group list-group-flush file-list-map">
											@if(file_exists(Storage::path('public/uploads/perumahan/peta/'.$perumahan->id)) and is_dir(Storage::path('public/uploads/perumahan/peta/'.$perumahan->id)))
											@foreach(Illuminate\Support\Facades\File::allFiles(Storage::path('public/uploads/perumahan/peta/'.$perumahan->id)) as $i => $file)
											<li class="list-group-item d-flex justify-content-between">
												<a href="{{ asset(Storage::url('public/uploads/perumahan/peta/'.$perumahan->id.'/'.$file->getFilename())) }}" target="_blank">{{$file->getFilename()}}</a>
												<button type="button" class="btn btn-danger btn-sm btn-remove-file" data-id="{{ $perumahan->id }}" data-file="{{ $file->getFilename() }}"><i class="fa fa-trash"></i></button></li>
											</li>
											@endforeach
											@endif
                                        </ul>
...
<script>
    ...
    $(".btn-generate-map").on("click",function(){
        ...
        var url = "{{ route('admin.perumahan.generateShp',['perumahan' => $perumahan])}}";
        var data = [];
        $.each(file_list,function(i,v){
            data.push($(v).find(".btn-remove-file").data("file"));
        });
    ...
    $(".btn-save-map").on("click",function(){
        var file_list = $(".file-list-map li");
        var url = "{{ route('admin.perumahan.saveGeojson',['perumahan' => $perumahan])}}";
        var data = [];
    ...
    $(".dropzone-peta").dropzone({
        url: '{{ route('admin.perumahan.uploadPeta', ['perumahan' => $perumahan]) }}',
        maxFilesize: 512, // MB
        dataType: "json",
        addRemoveLinks: true,
        headers: {
        ...
        success: function(file, response) {
            ...
            addFileToListMap(file,$(this)[0].clickableElements,response,{{$perumahan->id ?? 0}});
        },

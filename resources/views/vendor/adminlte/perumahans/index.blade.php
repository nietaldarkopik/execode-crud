...
<script>
    ...
    function savePsuDetail(form,id,callback) {
        var url = "{{ route('admin.psuperumahan.updatePsuItem',['id' => 'xx'])}}";
    ...
    function addFileToListMap(path,clickableElements,response,id_perumahan){
        ...
        //var geojsonLayer = new L.GeoJSON.AJAX(path); // Ganti path dengan path GeoJSON Anda
        //geojsonLayer.addTo(map);
        $(".file-list-map").prepend('<li class="list-group-item d-flex justify-content-between"><a href="{{ asset('/') }}'+path.name+'" target="_blank">'+path.name+'</a><button type="button" class="btn btn-danger btn-sm btn-remove-file" data-id="'+id_perumahan+'" data-file="'+path.name+'"><i class="fa fa-trash"></i></button></li>');
    }
	...
	$("body").on("click",".btn-remove-file",function(){
		if(confirm("Apakah yakin ingin menghapus file ini?"))
		{
			...
			var url = "{{ route('admin.perumahan.removeFilePeta',['perumahan' => 'xx'])}}";
			url = url.replace('xx',id);
	...
	$("body").on("click",".btn-remove-file-document",function(){
		if(confirm("Apakah yakin ingin menghapus file ini?"))
		{
			...
			var file = $(this).data('file');
			var url = "{{ route('admin.perumahan.removeFileDocument',['perumahan' => 'xx'])}}";
			url = url.replace('xx',id);
			var li = $(this).closest('li');
	...
	$("body").on("click",".btn-remove-psu",function(){
		if(confirm("Apakah yakin ingin menghapus data ini?"))
		{
			...
			var url = "{{ route('admin.perumahan.removePsu',['perumahan' => 'xx','id_psu' => 'yy'])}}";
			url = url.replace('xx',id);
			url = url.replace('yy',id_psu);

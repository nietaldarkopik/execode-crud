...
<a href="<?php echo e($dashboard_url); ?>"
	...
	@php
		$web_title = DB::table('settings')->where('global.web_title')->first();
		$logo = DB::table('settings')->where('global.web_logo')->first();
		$logo_img = (isset($logo->value) && !empty($logo->value))?asset(Storage::path($logo->value)):asset(config('adminlte.logo_img', 'vendor/adminlte/dist/img/AdminLTELogo.png'));
		$web_title = (isset($web_title->value) && !empty($web_title->value))?$web_title->value:config('adminlte.logo_img_alt', 'AdminLTE');
	@endphp
...
<?php /**PATH C:\wamp\www\laravel-scheduler\resources\views/vendor/adminlte/partials/common/brand-logo-xs.blade.php ENDPATH**/ ?>